 ...
 * @subpackage Mwb_Cf7_Integration_With_Salesforce_Crm/mwb-crm-fw
 */
class Mwb_Cf7_Integration_Salesforce_Api_Base extends Mwb_Cf7_Integration_Api_Base {
	...
	/**
	 * Main Mwb_Sf_Cf7_Integration_Salesforce_Api_Base Instance.
	 *
	 * Ensures only one instance of Mwb_Sf_Cf7_Integration_Salesforce_Api_Base is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 *
	 * @static
	 * @return Mwb_Sf_Cf7_Integration_Salesforce_Api_Base - Main instance.
	 */
	...
	public static function initialize( $token_data = array() ) {
		...
		self::$crm_prefix = Mwb_Cf7_Integration_With_Salesforce_Crm::get_current_crm( 'slug' );

		if ( empty( $token_data ) ) {
			$token_data = get_option( 'mwb-' . self::$crm_prefix . '-cf7-token-data', array() );
		}

		self::$enviornment = get_option( 'mwb-' . self::$crm_prefix . '-cf7-enviornment', '' );
		self::$own_app     = get_option( 'mwb-' . self::$crm_prefix . '-cf7-integration-own-app', false );

		if ( false != self::$own_app && 'yes' == self::$own_app ) { // phpcs:ignore
			self::$consumer_key    = get_option( 'mwb-' . self::$crm_prefix . '-cf7-consumer-key', '' );
			self::$consumer_secret = get_option( 'mwb-' . self::$crm_prefix . '-cf7-consumer-secret', '' );
			...
			self::$redirect        = 'https://auth.makewebbetter.com/integration/salesforce-auth/';
		}
	...
	public function save_company_id( $company_id = false ) {
		...
		update_option( 'mwb-' . self::$crm_prefix . '-cf7-company-id', $company_id );
	}
	...
	public function get_auth_code_url() {
		...
		$query_args = array(
			...
			'client_id'     => urlencode( self::$consumer_key ), // phpcs:ignore
	...
	public function get_oauth_state( $use_custom_app = 'no' ) {
		...
		$nonce = wp_create_nonce( 'mwb_' . self::$crm_prefix . '_state' );
	...
	public function process_access_token( $code = '', $refresh_token = '' ) {
		...
		if ( isset( $response['code'] ) && 200 == $response['code'] ) { // phpcs:ignore
			...
			update_option( 'mwb-' . self::$crm_prefix . '-cf7-token-data', $token_data );
			update_option( 'mwb-' . self::$crm_prefix . '-cf7-crm-connected', true );
			self::initialize( $token_data );
		} else {
			// On failure add to log.
			delete_option( 'mwb-' . self::$crm_prefix . '-cf7-token-data' );
			delete_option( 'mwb-' . self::$crm_prefix . '-cf7-crm-connected' );
		}
	...
	public function merge_refresh_token( $new_token_data ) {
		...
		$old_token_data = get_option( 'mwb-' . self::$crm_prefix . '-cf7-token-data', array() );
	...
	public function prepare_bulk_request_data( $object, $record_data ) {
		...
		foreach ( $record_data as $key => $data ) {
			...
			if ( ! empty( $data['id'] ) ) {
				...
				// Unique indexes can not be sent in update request.
				if ( 'PricebookEntry' === $object ) {
					unset( $data['Pricebook2Id'] );
					unset( $data['Product2Id'] );
				}
				if ( 'Product2' === $object ) {
					if ( isset( $data['StockKeepingUnit'] ) ) {
						unset( $data['StockKeepingUnit'] );
					}
				}
	 ...
	 * @return   mixed
	 */
	public function bulk_create_or_update_record( $object, $record_data, $primary_field ) {
	...
	public function create_or_update_record( $object, $record_data, $is_bulk = false, $log_data = array(), $manual_sync = false ) {
		...
		$response_data = array(
			'success' => false,
			'msg'     => __( 'Something went wrong', 'mwb-cf7-integration-with-salesforce-crm' ),
		);
		...
		if ( $feed_id ) {
			$duplicate_check_fields = get_post_meta( $feed_id, 'mwb-' . self::$crm_prefix . '-cf7-primary-field', true );
		...
		if ( ! $record_id ) {
			...
			if ( $this->is_success( $response ) ) {
				...
				$response_data['msg']      = esc_html__( 'Error posting to CRM', 'mwb-cf7-integration-with-salesforce-crm' );
				$response_data['response'] = $response;
			}
		} else {
	...
	public function get_crm_objects( $force = false ) {
		...
		$method  = get_option( 'mwb-' . self::$crm_prefix . '-cf7-integration-method', false );
		$objects = array();

		switch ( $method ) {
			case 'api':
				$data = get_transient( 'mwb_salesforce_cf7_objects_data' );
				if ( ! $force && false !== ( $data ) ) {
					return $data;
				}
				...
				if ( ! empty( $response ) ) {
					...
					if ( isset( $response['code'] ) && 200 == $response['code'] && 'OK' == $response['message'] ) { // phpcs:ignore
						if ( ! empty( $response['data'] ) && ! empty( $response['data']['sobjects'] ) ) {
							...
							set_transient( 'mwb_salesforce_cf7_objects_data', $objects );
						}
					}
				}
				break;
	...
	public function get_object_fields( $object, $force = false ) {
		...
		if ( $method ) {
			switch ( $method ) {
				case 'api':
					$data = get_transient( 'mwb_salesforce_cf7' . $object . '_fields' );
					if ( ! $force && false !== ( $data ) ) {
						return $data;
					}
					...
					if ( ! empty( $response ) ) {
						...
						if ( isset( $response['code'] ) && 200 == $response['code'] && 'OK' == $response['message'] ) { // phpcs:ignore
							if ( ! empty( $response['data'] ) && ! empty( $response['data']['fields'] ) ) {
								...
								set_transient( 'mwb_salesforce_cf7' . $object . '_fields', $fields );
							}
						}
					}
					break;
				...
				case 'web':
					...
					$fields_data = array(
						'Lead' => $wp_filesystem->get_contents( MWB_CF7_INTEGRATION_WITH_SALESFORCE_CRM_DIRPATH . '/mwb-crm-fw/framework/json/lead.json' ), // phpcs:ignore
						'Case' => $wp_filesystem->get_contents( MWB_CF7_INTEGRATION_WITH_SALESFORCE_CRM_DIRPATH . '/mwb-crm-fw/framework/json/case.json' ), // phpcs:ignore
					);
	...
	public function post_via_web( $auth = false, $object = 'Lead', $data = array(), $log_data = array() ) {
		...
		$web = array(
			'orgid'       => get_option( 'mwb-' . self::$crm_prefix . '-cf7-salesforce-orgid', false ),
			'domain'      => get_option( 'mwb-' . self::$crm_prefix . '-cf7-salesforce-domain', false ),
			'environment' => get_option( 'mwb-' . self::$crm_prefix . '-cf7-enviornment', false ),
		);
		...
		$header = array(
			'user-agent' => 'MWB CF7 Integration with Salesforce Plugin - WordPress/' . $wp_version . '; ' . get_bloginfo( 'url' ),
		);
		$body   = array();
		...
		if ( false === $auth ) {
			if ( isset( $response['code'] ) && 200 == $response['code'] ) { // phpcs:ignore
				$count = get_option( 'mwb-' . self::$crm_prefix . '-cf7-synced-forms-count', 0 );
				update_option( 'mwb-' . self::$crm_prefix . '-cf7-synced-forms-count', $count + 1 );
			}
