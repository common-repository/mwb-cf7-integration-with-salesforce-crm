 ...
 * @author     MakeWebBetter <https://makewebbetter.com>
 */
class Mwb_Cf7_Integration_Salesforce_Template_Manager {
		...
		if ( $this->admin::pro_dependency_check() ) {
			...
			if ( $pro_main::$license() || 0 <= $days_count ) {
				if ( ! $pro_main::$license() && 0 <= $days_count ) {
					...
					$day_string = sprintf( _n( '%s day', '%s days', $warning, 'mwb-cf7-integration-with-salesforce-crm' ), number_format_i18n( $warning ) );
					?>
					<div id="mwb-sf-cf7-thirty-days-notify" class="notice notice-warning mwb-notice">
						<p>
							<strong><a href="<?php echo esc_url( admin_url( 'admin.php' ) . '?page=mwb_' . $this->crm_slug . '_cf7_page&tab=license' ); ?>">

							<!-- License warning. -->
							<?php esc_html_e( 'Activate', 'mwb-cf7-integration-with-salesforce-crm' ); ?></a>
							<?php
							/* translators: %s is replaced with "days remaining" */
							printf( esc_html__( ' the license key before %s or you may risk losing data and the plugin will also become dysfunctional.', 'mwb-cf7-integration-with-salesforce-crm' ), '<span id="mwb-sf-cf7-day-count" >' . esc_html( $day_string ) . '</span>' );
							?>
							</strong>
						</p>
					</div>
				...
				<div id="mwb-sf-cf7-thirty-days-notify" class="notice notice-warning mwb-notice">
					<p>
						<strong>
							<?php esc_html_e( 'Trail expried !! ', 'mwb-cf7-integration-with-salesforce-crm' ); ?></a>
							...
							<?php
							/* translators: %s is replaced with "days remaining" */
							printf( esc_html__( ' your license and continue enjoying the pro version features.', 'mwb-cf7-integration-with-salesforce-crm' ) );
							?>
						</strong>
					</p>
				</div>
	...
	public function get_nav_tabs() {
		...
		/* Tabs section start. */
		?>
		<?php if ( ! mwb_sf_cf7_is_plugin_active( 'cf7-integration-with-' . $this->crm_slug . '-crm/cf7-integration-with-' . $this->crm_slug . '-crm.php' ) ) : ?>
		<div class="mwb-about__list-item-btn mwb-floating-btn">
			<span class="dashicons dashicons-star-filled mwb-premium-icon"></span>
			<a href="<?php echo esc_url( 'https://makewebbetter.com/product/cf7-integration-with-salesforce/?utm_source=MWB-salesforce-org&utm_medium=MWB-site-backend&utm_campaign=MWB-salesforce-site' ); ?>" target="_blank" class="mwb-btn mwb-btn--filled">
				<?php esc_html_e( 'Go Premium', 'mwb-cf7-integration-with-salesforce-crm' ); ?>
			</a>
		</div>
		<?php endif; ?>

		<nav class="mwb-sf-cf7-navbar">
			<div class="mwb-sf-cf7-nav-collapse">
				<ul class="mwb-sf-cf7-nav mwb-sf-cf7-nav-tabs" role="tablist">
					<?php $tabs = $this->retrieve_nav_tabs(); ?>
					<?php if ( ! empty( $tabs ) && is_array( $tabs ) ) : ?>
						<?php foreach ( $tabs as $href => $label ) : ?>
							<li class="mwb-sf-cf7-nav-item">
								<a class="mwb-sf-cf7-nav-link nav-tab <?php echo esc_html( $active_tab == $href ? 'nav-tab-active' : '' ); // phpcs:ignore ?>" href="?page=mwb_<?php echo esc_html( $this->crm_slug ); ?>_cf7_page&tab=<?php echo esc_html( $href ); ?>"><?php echo esc_html( $label ); ?></a>
							</li>
						<?php endforeach; ?>
					<?php endif; ?>
				</ul>
	...
	public function render_authorisation_screen() {
		...
		$params['consumer_key']    = get_option( 'mwb-' . $this->crm_slug . '-cf7-consumer-key' );
		$params['consumer_secret'] = get_option( 'mwb-' . $this->crm_slug . '-cf7-consumer-secret' );
		$params['environment']     = get_option( 'mwb-' . $this->crm_slug . '-cf7-enviornment' );
		$params['connection']      = get_option( 'mwb-' . $this->crm_slug . '-cf7-connection-data' );
		$params['orgid']           = get_option( 'mwb-' . $this->crm_slug . '-cf7-salesforce-orgid' );
		$params['domain']          = get_option( 'mwb-' . $this->crm_slug . '-cf7-salesforce-domain' );
		$params['own_app']         = get_option( 'mwb-' . $this->crm_slug . '-cf7-integration-own-app' );
	...
	public function retrieve_nav_tabs() {
		...
		switch ( $current_screen ) {
			...
			case 'mwb_' . $this->crm_slug . '_cf7_page':
				$tabs = array(
					'accounts' => esc_html__( 'Dashboard', 'mwb-cf7-integration-with-salesforce-crm' ),
					'feeds'    => esc_html__( 'Feeds', 'mwb-cf7-integration-with-salesforce-crm' ),
					'logs'     => esc_html__( 'Logs', 'mwb-cf7-integration-with-salesforce-crm' ),
					'settings' => esc_html__( 'Settings', 'mwb-cf7-integration-with-salesforce-crm' ),
				);
				break;
		}
	...
	protected function load_template( $template = '', $params = array() ) {
		...
		if ( 'license' == $template ) { // phpcs:ignore
			if ( $this->admin::pro_dependency_check() ) {
				require_once CF7_INTEGRATION_WITH_SALESFORCE_CRM_DIRPATH . 'mwb-crm-fw/framework/templates/tab-content/license-tab.php';
			}
		} else {
			$params['admin_class'] = $this->admin;
			$params['method']      = get_option( 'mwb-' . $this->crm_slug . '-cf7-integration-method' );
	...
	public function add_plugin_links() {
		...
		$links = array(
			'doc'     => 'https://docs.makewebbetter.com/mwb-cf7-integration-salesforce-crm/?utm_source=org&utm_medium=plugin-backend&utm_campaign=salesforce',
			'ticket'  => 'https://makewebbetter.com/submit-query/?utm_source=org&utm_medium=plugin-backend&utm_campaign=salesforce',
			'contact' => 'https://makewebbetter.com/contact-us/?utm_source=org&utm_medium=plugin-backend&utm_campaign=salesforce',
		);
	...
	public static function get_field_section_html(
		...
		$method       = get_option( 'mwb-' . $crm_slug . '-cf7-integration-method' );
		...
		<div class="mwb-feeds__form-wrap mwb-fields-form-row">
			<div class="mwb-form-wrapper">
				...
				<div class="mwb-fields-form-section-head">
					...
					<?php if ( isset( $fields_data['mandatory'] ) && ! $fields_data['mandatory'] ) : ?>
						<span class="field-delete dashicons">
							<img src="<?php echo esc_url( MWB_CF7_INTEGRATION_WITH_SALESFORCE_CRM_URL . 'admin/images/trash.svg' ); ?>" style="max-width: 20px;">
						</span>
					<?php elseif ( isset( $fields_data['display'] ) && $fields_data['display'] ) : ?>
						<span class="field-delete dashicons">
							<img src="<?php echo esc_url( MWB_CF7_INTEGRATION_WITH_SALESFORCE_CRM_URL . 'admin/images/trash.svg' ); ?>" style="max-width: 20px;">
						</span>
					<?php endif; ?>
				...
				<div class="mwb-fields-form-section-meta">
					<span>
						<?php 'api' == $method ? esc_html_e( 'API Name : ', 'mwb-cf7-integration-with-salesforce-crm' ) : esc_html_e( 'Name : ', 'mwb-cf7-integration-with-salesforce-crm' ); // phpcs:ignore ?>
						<?php echo esc_html( $fields_data['name'] ); ?>
					</span>
					<span>
						<?php esc_html_e( 'Type : ', 'mwb-cf7-integration-with-salesforce-crm' ); ?>
						<?php echo esc_html( $fields_data['type'] ); ?> 
					</span>
					<span>
						<?php if ( ! empty( $fields_data['length'] ) ) : ?>
							<?php esc_html_e( 'Length : ', 'mwb-cf7-integration-with-salesforce-crm' ); ?>
							<?php echo esc_html( ! empty( $fields_data['length'] ) ? $fields_data['length'] : '' ); ?> 
						<?php endif; ?>
					</span>
					<?php if ( ! empty( $fields_data['picklistValues'] ) ) : ?>
						<span>
							<?php esc_html_e( 'Picklist Values : ', 'mwb-cf7-integration-with-salesforce-crm' ); ?><?php echo esc_html( implode( ', ', array_column( $fields_data['picklistValues'], 'label' ) ) ); ?> 
						</span>
					<?php endif; ?>
					<?php if ( isset( $fields_data['mandatory'] ) && $fields_data['mandatory'] ) : ?>
						<span class="mwb-required-tag">
							<b><?php echo sprintf( '%s ', esc_html__( 'Required Field', 'mwb-cf7-integration-with-salesforce-crm' ) ); ?></b>
						</span>
					<?php endif; ?>
				</div>
				...
				<div class="mwb-fields-form-section-form">
					...
					<div class="form-field-row row-field-type">
						<label>
							<?php esc_html_e( 'Field Type', 'mwb-cf7-integration-with-salesforce-crm' ); ?>
						</label>
						<select class="field-type-select" name="field_type[]">
							<option value=""><?php esc_html_e( 'Select an Option', 'mwb-cf7-integration-with-salesforce-crm' ); ?></option>
							<option value="standard_field" <?php echo esc_attr( selected( 'standard_field', $default_data['field_type'] ) ); ?> >
								<?php esc_html_e( 'Standard Value', 'mwb-cf7-integration-with-salesforce-crm' ); ?>
							</option>
							<?php if ( ! $require_pricebook_mapping ) : ?>
								<option value="custom_value" <?php echo esc_attr( selected( 'custom_value', $default_data['field_type'] ) ); ?>>
									<?php esc_html_e( 'Custom Value', 'mwb-cf7-integration-with-salesforce-crm' ); ?>		
								</option>
							<?php endif; ?>
						</select>
					</div>
					...
					<div class="form-field-row row-field-value row-standard_field <?php echo esc_attr( $row_stndrd ); ?>">
						<label><?php esc_html_e( 'Field Value', 'mwb-cf7-integration-with-salesforce-crm' ); ?></label>
					...
					<?php if ( isset( $default_data['use_status_mapping'] ) && $require_status_mapping ) : ?>
						<div class="form-field-row">
							<label><?php esc_html_e( 'Map with woo order statuses', 'mwb-cf7-integration-with-salesforce-crm' ); ?></label>
	...
	public function render_and_conditon( $and_condition = array(), $and_index = '1', $or_index = '' ) {
		...
		<div class="and-condition-filter" data-and-index=<?php echo esc_attr( $and_index ); ?> >
			<select name="condition[<?php echo esc_html( $or_index ); ?>][<?php echo esc_html( $and_index ); ?>][field]"  class="condition-form-field" >
				<option value="-1" ><?php esc_html_e( 'Select Field', 'mwb-cf7-integration-with-salesforce-crm' ); ?></option>
			...
			<select name="condition[<?php echo esc_html( $or_index ); ?>][<?php echo esc_html( $and_index ); ?>][option]" class="condition-option-field" >
				<option value="-1"><?php esc_html_e( 'Select Condition', 'mwb-cf7-integration-with-salesforce-crm' ); ?></option>
			...
			<input type="text" name="condition[<?php echo esc_html( $or_index ); ?>][<?php echo esc_html( $and_index ); ?>][value]" class="condition-value-field" value="<?php echo esc_html( ! empty( $and_condition['value'] ) ? $and_condition['value'] : '' ); ?>" placeholder="<?php esc_html_e( 'Enter value', 'mwb-cf7-integration-with-salesforce-crm' ); ?>">
			...
			<li class="mwb-sf-cf7__feed-row">
				<div class="mwb-sf-cf7__left-col">
					...
					<p>
						...
						<span><?php echo esc_html( 'yes' != $filter_applied ? '-' : esc_html__( 'Applied', 'mwb-cf7-integration-with-salesforce-crm' ) ); // phpcs:ignore ?></span> 
					</p>
				</div>
				<div class="mwb-sf-cf7__right-col">
					<a href="<?php echo esc_url( $edit_link ); ?>"><img src="<?php echo esc_url( MWB_CF7_INTEGRATION_WITH_SALESFORCE_CRM_URL . 'admin/images/edit.svg' ); ?>" alt="<?php esc_html_e( 'Edit feed', 'mwb-cf7-integration-with-salesforce-crm' ); ?>"></a>
					<div class="mwb-sf-cf7__right-col1">
						<a href="javascript:void(0)" class="mwb_<?php echo esc_attr( $this->crm_slug ); ?>_cf7__trash_feed" feed-id="<?php echo esc_html( $feed->ID ); ?>"><img src="<?php echo esc_url( MWB_CF7_INTEGRATION_WITH_SALESFORCE_CRM_URL . 'admin/images/trash.svg' ); ?>" alt="<?php esc_html_e( 'Trash feed', 'mwb-cf7-integration-with-salesforce-crm' ); ?>"></span></a>
					</div>
				</div>
			</li>
			<?php
